...
鼠标滚轮事件由 `WH_MOUSE_LL` 低级钩子全局捕获，仅在鼠标悬停托盘图标后 2 秒窗口内生效。每次滚轮事件自动续期窗口，持续滚动永不过期。非任务栏区域点击鼠标会立即中断窗口并隐藏 OSD。

```
HookCallback → PostMessage(WM_APP_WHEEL) → HandleWheel
```

**HandleWheel 处理流程：**

- 步进固定 **±2%**（每次一格）
- `_pendingDelta` 累加步进，供后续 OSD 数值显示
- OSD 即时弹出 **▲**（增大）或 **▼**（减小）方向符号
- 正常模式下：计算目标音量 → 16ms 防抖定时器批量写入 DAC
- 音量未知时（启动未读完设备）→ 跳过，不操作 DAC
- 重启 500ms OSD 数值显示定时器

**静音状态特殊处理：**

| 操作 | 行为 |
|------|------|
| 已静音 + 上滚 | `AdjustVolume()` 用静音前记忆值恢复并解除静音 |
| 已静音 + 下滚 | 不操作 DAC，保持静音 |
...
|------|----------|----------|
| 滚动中 | ▲ / ▼ 方向符号 | 每次滚轮刷新 |
| 停止 500ms | 实际音量数值（如 `48%`） | 800ms 后消失 |
| 单击静音 | `静音` 或音量值 | 800ms 后消失 |

### 静音切换

```
左键单击托盘图标
  ├─ 已静音 → 恢复到静音前音量（或 10% 兜底）
  └─ 未静音 → 设为 0%，记忆当前音量
```

单击时会先停掉 16ms 防抖定时器，防止上一轮滚轮目标值覆盖静音操作。

### HID 设备通信

**写入（SetVolumeInternal）**：同步发送 Feature Report，成功则更新缓存 `_volume`，失败则断开设备。

**读取（ReadVolume / ReadVolumeAsync）**：

1. 排空输入缓冲区（5ms × 10 次，丢弃旧报告）
2. 发送读命令（reportId `0x4B`）
3. 读取响应，最多重试 5 次
4. 成功 → 更新 `_volume`；超时/异常/无匹配 → `_volume = -1`（标记未知）

**启动流程**：`MainForm_Load` 中异步连接设备并 `await` 读取初始音量，读完才显示托盘图标。

### 防错保护

| 场景 | 行为 |
|------|------|
| 音量未知时滚轮 | 不操作，托盘显示「读取中…」 |
| HID 写失败 | 断开设备，下次滚轮自动重连 |
| HID 读失败 | 标记未知，不基于过期缓存调音量 |
| 全屏前台窗口 | 暂停鼠标钩子（防反作弊误判） |
| 离开图标 > 2s | 滚轮失效，移回图标自动恢复 |
| 非任务栏点击 | 中断滚轮窗口 + 隐藏 OSD |
